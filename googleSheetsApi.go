...
func getMatchesFromSheet()([]Match, error){
	var output = []Match{}
	...
	for i:=1; i<lastRowId; i++{
		...
		if err == nil {
			...
			return []Match{}, errors.New("Invalid match date: "+sheet.Rows[i][3].Value)
		}
	}
	return output,nil
}
...
func checkMatchesHeader(sheet *spreadsheet.Sheet)(bool){
	if len(sheet.Rows[0]) < 4 {
		return false
	} else if sheet.Rows[0][0].Value != "idx" || sheet.Rows[0][1].Value != "team1" || sheet.Rows[0][2].Value != "team2" || sheet.Rows[0][3].Value != "date" {
		return false
	}
	return true
}
...
func getLastEntry(parameterList []string, wnumber string, serviceId string, userTableTitle string) ([]string) {
	sheet, err := sprsheet.SheetByIndex(0)
...
func getLastValidRowId(sheet *spreadsheet.Sheet, wnumber string) (int, bool) {
	...
	for i := lastEmptyRow; i >= 1; i -= 1 {
		...
		log.Println("sheet.Rows[i][3].Value", sheet.Rows[i][3].Value)
		if sheet.Rows[i][3].Value == wnumber { // search for our latest entry
			return i, true
		}
	}
	if sheet.Rows[prevI][3].Value == wnumber {
		return prevI, true
	} else {
		return 0, false
	}
